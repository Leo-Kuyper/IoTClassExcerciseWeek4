...
name = "Leo Kuyper" #Update Name and Surname
# Update these values!# Mongo endpoints and setup
JSON_GET_URL = "http://" + serverIp + "/api/getRelay/" + name
JSON_POST_URL = "http://" + serverIp + "/api/addLight/"
...
while True:
    ...
    try:
        ...
        if timeElapsed > TIME_BETWEEN_QUERY:
            ...
            print("POSTing data to {0}: {1}".format(JSON_POST_URL, light))
            while not response:
                try:
                    json_data = {"name": name, "light": light,}
            ...
            print("-" * 40)
            print("JSON Data received from server:", json_resp["light"])
            print("-" * 40)
            response.close()
            response = None
        ...
        if not json:
            ...
            length = len(json)
            print("JSON Response: Relay  ", json['relay'])
            relay.value = json['relay']
